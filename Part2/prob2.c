...
int main (int argc, char *argv[]){
    ...
    if (rank == 0) {                     /* dispatcher process it is the first process of the group */
        ...
        filesManager = (FILEINFO*) calloc(numFiles, sizeof(FILEINFO));
        ...
        while (filePos <= numFiles) {
            ...
            filesManager[filePos - 1].numSamples = samples;
...
static void computeValue(double *x, double *y, CONTROLINFO *ci) {
   ...
   size_t i;
   size_t n = ci->numSamples;
   size_t point = ci->rxyIndex;

   for(i = 0; i < n; i++){
      ci->result += x[i] * y[(point+i)%n];
   }
}
...
static void printResults(unsigned int numFiles, char** filesToProcess){
  ...
  size_t i, x, numErrors;

  for (i = 0; i < numFiles; i++){
    numErrors = 0;
    for (x = 0; x < filesManager[i].numSamples; x++){
...
static void savePartialResults(CONTROLINFO *ci) {
  ...
  ci->result = 0;
  if(filesManager[ci->filePosition].rxyIndex == filesManager[ci->filePosition].numSamples){
    ci->filePosition++;
    ci->processing = false;
  }
}
